...
export class BlockchainService {
  ...
  private getSigner(wallet: ethers.Wallet): ethers.Contract {
    if (!this.donationContract) {
      throw new Error('Donation contract not initialized');
    }
    return this.donationContract.connect(wallet);
  }
  ...
  async donateNative(
    ...
    const contract = this.getSigner(wallet);
  ...
  async donateUSDC(
    ...
    // Then donate
    const contract = this.getSigner(wallet);
    const tx = await contract.donateUSDC(recipientAddress, amountInWei);
    return tx;
  }
  ...
  async withdrawNative(
    ...
    const contract = this.getSigner(wallet);
    const tx = await contract.withdrawNative(ethers.parseEther(amount));
    return tx;
  }
  ...
  async withdrawUSDC(
    ...
    const decimals = await this.usdcContract.decimals();
    const contract = this.getSigner(wallet);
    const tx = await contract.withdrawUSDC(ethers.parseUnits(amount, decimals));
    return tx;
  }
